...
election_title <- "2024 US Presidential Election"
...
How much did campaigns spend on different targeting methods? The graphs below show spending *overall* across all entities and *by entity*.
  ...
  mutate(spend_perlab = scales::comma(round(spend_per))) %>% 
    ...
    map(~ {
      ...
      hchart(.x, type = "bar", hcaes(x = target, y = perc), color = the_color) %>%
            ...
            hc_yAxis(title = list(text = "% Budget spent on targeting method"), max = 100, min = 0) %>%
...
Here, we show the *top targeted audiences* for each entity for the "detailed" targeting that Meta offers (ranked by amount of Euro spend). Most (but not all) of these audiences are “interest” audiences, i.e. the parties targeted people interested in “Books” or “Politics”.
  ...
  plots <- gg %>%
    ...
    map(~ {
      ...
      hchart(.x,
        ...
        hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
        hc_yAxis(
          title = list(text = paste0("% Budget spent on ", wooord2, " audience")),
          max = max(.x$perc)#,
          # min = 0
        ) %>%
        hc_xAxis(title = "", labels = list(style = list(fontSize = "10px"))) %>%
...
The graphs below show targeting by age groups and how much percentage of Budgets were spent on them.
...
The graphs below show targeting by gender and how much percentage of Budgets were spent on them.
...
charts <- gender_targeting %>%
  ...
  map(~ {
    ...
    hchart(.x, type = "bar", hcaes(x = value, y = perc), color = the_color) %>%
      hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
      hc_yAxis(title = list(text = "% Budget spent on targeting method"),
...
The graphs below show targeting by education levels and how much percentage of Budgets were spent on them.
...
The graphs below show targeting by job (sectors) and how much percentage of Budgets were spent on them.
